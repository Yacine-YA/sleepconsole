...
void CSkeletonBasics::hand_cordinates_to_file(int shoulder, int elbow, int wrist, int hand,string side) {
	file << time(0) << "\t"<<side<<"\t" << shoulder << "\t" << elbow << "\t" << wrist << "\t" << hand << "\n";
...
{      
	...
	//hand_cordinates_to_file(m_Points[NUI_SKELETON_POSITION_SHOULDER_LEFT], NUI_SKELETON_POSITION_ELBOW_LEFT, NUI_SKELETON_POSITION_WRIST_LEFT, NUI_SKELETON_POSITION_HAND_LEFT, "LEFT ARM");
    // Right Arm


	file << time(0) << "-->" << m_Points[NUI_SKELETON_POSITION_SHOULDER_LEFT].x<<" "<<m_Points[NUI_SKELETON_POSITION_SHOULDER_LEFT].y << "\t" << m_Points[NUI_SKELETON_POSITION_ELBOW_LEFT].x<<" "<<m_Points[NUI_SKELETON_POSITION_ELBOW_LEFT].y << "\t" << m_Points[NUI_SKELETON_POSITION_WRIST_LEFT].x<<" "<<m_Points[NUI_SKELETON_POSITION_WRIST_LEFT].y << "\t" << m_Points[NUI_SKELETON_POSITION_HAND_LEFT].x<<" "<<m_Points[NUI_SKELETON_POSITION_HAND_LEFT].y<<"\n";
    ...
    for (i = 0; i < NUI_SKELETON_POSITION_COUNT; ++i)
    {
        ...
        if ( skel.eSkeletonPositionTrackingState[i] == NUI_SKELETON_POSITION_INFERRED )
        {
			...
			cout << m_pBrushJointInferred <<"\n";

        }
        else if ( skel.eSkeletonPositionTrackingState[i] == NUI_SKELETON_POSITION_TRACKED )
        {
            m_pRenderTarget->DrawEllipse(ellipse, m_pBrushJointTracked);
			cout << m_pBrushJointTracked << "\n";
